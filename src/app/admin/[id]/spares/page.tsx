...
async function fetchSpares(baseUrl: string, machineId: string): Promise<SpareRow[]> {
  try {
    const r = await fetch(`${baseUrl.replace(/\/$/, "")}/api/spares/${machineId}`, {
      cache: "no-store",
    });
    if (!r.ok) return [];
...
export default async function AdminMachineSparesPage({
  params,
}: {
  ...
  const rows = await fetchSpares(baseUrl, machine.machineId);

  const summary = computeSummary(rows);
  const stats = [
    { label: "Total Spares Installed", value: summary.totalInstalledQty },
    ...
    <div className="space-y-4">
      <div className="space-y-1">
        <div className="text-sm text-black/50">
          ...
          <span className="text-black font-medium">Spares Installation Reports</span>
        </div>
